...
#     template = loader.get_template('home.html')
#     user = request.user
#     context = {'user_group': user.groups.first().name}
#     return HttpResponse(template.render(context, request))
...
def ProjectForm(request):
    ...
    if request.method == 'POST':
        ...
        if prjform1.is_valid() and prjform2.is_valid():
            ...
            # prjform1.save()  # save the object if both forms are valid
            # prjform2.save()
...
def SrjrMapFormView(request):
    form = SrjrMapForm(request.user)
    # logged_in_user = request.user
    # empdata = Employee.objects.exclude(user=logged_in_user)
    # empdata = Employee.objects.filter(user=request.user)
    empdata = Employee.objects.all()
    # userdata = User.objects.exclude(user=request.user)
...
def SrjrEntry(request):
    data = Mapping.objects.filter(user=request.user)
...
# @never_cache
# def AttForm(request):
#     att = Attendance()  # create an object for the shared model
#     map_data = Mapping.objects.filter(user=request.user)

#     if request.method == 'POST':
#         attform1 = AttForm1(request.POST, instance=att)  # pass the object to both forms
#         attform2 = AttForm2(request.POST, instance=att)
#         if attform2.is_valid():
#             att = attform1.save(commit=False)
#             att = attform2.save(commit=False)
...
#             return redirect('AttEntry')
#     else:
#         attform1 = AttForm1(instance=att)  # pass the object to both forms
#         attform2 = AttForm2(instance=att)

#     juniors = Mapping.objects.filter(user=request.user)
#     return render(request, "AttForm.html", {'attform1': attform1, 'attform2': attform2, 'juniors': juniors, 'map_data':map_data})
